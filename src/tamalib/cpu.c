...
void cpu_add_bp(breakpoint_t **list, u13_t addr)
{
	...
	if (!bp) {
		pd->system->error("Cannot allocate memory for breakpoint 0x%04X!\n", addr);
		return;
	}
...
static u4_t get_io(u12_t n)
{
	...
	switch (n) {
		...
		default:
			pd->system->error("Read from unimplemented I/O 0x%03X - PC = 0x%04X\n", n, pc);
	}
...
static void set_io(u12_t n, u4_t v)
{
	switch (n) {
		...
		default:
			pd->system->error("Write 0x%X to unimplemented I/O 0x%03X - PC = 0x%04X\n", v, n, pc);
	}
}
...
static u4_t get_memory(u12_t n)
{
	...
	if (n < MEM_RAM_SIZE) {
		...
		pd->system->error("Read from invalid memory address 0x%03X - PC = 0x%04X\n", n, pc);
		return 0;
	}
...
static void set_memory(u12_t n, u4_t v)
{
	...
	if (n < MEM_RAM_SIZE) {
		...
		//pd->system->error("Write 0x%X to invalid memory address 0x%03X - PC = 0x%04X\n", v, n, pc);
		return;
	}
...
static timestamp_t wait_for_cycles(timestamp_t since, u8_t cycles) {
	...
	deadline = since + (cycles * ts_freq)/(TICK_FREQUENCY);
	// optimize out multi
...
int cpu_step(void)
{
	...
	if (ops[i].log == NULL) {
		pd->system->error("Unknown op-code 0x%X (pc = 0x%04X)\n", op, pc);
		return 1;
	}
